...
public class EfunPushManager {
	...
	public void sendPushRequest(Context mContext) {
		...
		if (TextUtils.isEmpty(gamePreferredDomainUrl)) {
			try {
//				gamePreferredDomainUrl = EfunResourceUtil.findStringByName(mContext, "efunGamePreferredDomainUrl");
//				gameSpareDomainUrl = EfunResourceUtil.findStringByName(mContext, "efunGameSpareDomainUrl");
				gamePreferredDomainUrl = EfunResConfiguration.getGamePreferredUrl(mContext);
				gameSpareDomainUrl = EfunResConfiguration.getGameSpareUrl(mContext);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	*/
	...
	public EfunPushManager setNotifitionIcon(Context context,String notifitionIconName){
		PushSPUtil.savePullIcon(context, EfunResourceUtil.findDrawableIdByName(context, notifitionIconName));
		return getInstance();
	}
