...
import com.efun.core.tools.EfunLocalUtil.NetworkType;
...
/*		if (startId > 1) {
	...
	protected void resetClient() {
		//SharedPreferences account = this.getSharedPreferences(PushParams.DEFAULT_PRE_NAME, Context.MODE_PRIVATE);
		String serverIp = EfunDatabase.getSimpleString(this, EfunDatabase.EFUN_FILE, PushConstant.PUSH_SERVER_IP);//account.getString(PushParams.SERVER_IP, "");
		String serverPort = EfunDatabase.getSimpleString(this, EfunDatabase.EFUN_FILE, PushConstant.PUSH_SERVER_PORT);// account.getString(PushParams.SERVER_PORT, "");
...
//			editor.commit();
	...
	public static MessageDispatcher  instanceMessageDispatcher(Context context){
		if (dispatcher == null) {
			...
			if (!TextUtils.isEmpty(className)) {
				try {
					Class<com.efun.push.client.MessageDispatcher> dispatcherClazz = (Class<com.efun.push.client.MessageDispatcher>) Class.forName(className);
	...
	public class MyUdpClient extends UDPClientBase {
		...
		public boolean hasNetworkConnection() {
			int netType = EfunLocalUtil.getNetworkType(EfunPushService.this);
