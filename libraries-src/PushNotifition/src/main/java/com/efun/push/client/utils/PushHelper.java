...
public class PushHelper {
	...
	public static void processMessage(Context context, String message) {
		...
		String messageId = EfunDatabase.getSimpleString(context, EfunDatabase.EFUN_FILE, EFUN_PUSH_MESSAGE_ID);
		//出现相同messageid不显示消息
	...
	/*public static void notifyUser(Context context, NotificationMessage notificationMessage) {
		...
		// EfunLogUtil.logI("包名 ： " + pkName + "  className :" + className);

		int mIcon = PushSPUtil.takePullIcon(context, -1);
		EfunLogUtil.logI("mIcon id值为：" + mIcon);
		if (mIcon != -1 && mIcon != 0) {
...
//		notificationManager.notify(EfunPushService.getNotificationId(), notification);
		...
		EfunDatabase.saveSimpleInfo(context, EfunDatabase.EFUN_FILE, EFUN_PUSH_MESSAGE_ID, notificationMessage.getMessageId());
	...
	public static void setPushConfig(Context context, String serverIp, String serverPort) {
		...
		EfunDatabase.saveSimpleInfo(context, EfunDatabase.EFUN_FILE, PushConstant.PUSH_SERVER_IP, serverIp);
		EfunDatabase.saveSimpleInfo(context, EfunDatabase.EFUN_FILE, PushConstant.PUSH_SERVER_PORT, serverPort);
	}
	...
	public static String generateUUID(Context context) {
		return EfunStringUtil.toMd5(EfunLocalUtil.getDeviceType() + "_" + EfunLocalUtil.getLocalAndroidId(context), true);
	}
