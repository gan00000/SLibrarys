...
public class AlarmPushReceiver  {
	...
	public static void onReceive(Context context, Intent intent) {
		...
		if ("com.efun.alarmpush".equals(intent.getAction())) {
			SLog.logI("com.efun.alarmpush action");
...
//			int second = Integer.parseInt(d[2]);
			...
			SLog.logI(" week: "+week+" hour: "+hour+" minute: "+minute+" second: "+second);
			...
			if (stringArray != null) {
				for (int i = 0; i < stringArray.length; i++) {
					...
					if (week == pushweek) {
						SLog.logI("推送内容时间---->"+"pushhour:"+pushhour+"  pushminute:"+pushminute+"  messageContent:"+messageContent);
			...
			if (ymdList != null) {
				 for (int i = 0; i < ymdList.size(); i++) {
					SLog.logI("推送内容时间---->"+ymdList.get(i));
					...
					if (year == pyuer && month == pmonth && day == pday) {
						SLog.logI("推送内容时间---->"+"phour:"+phour+"  pminute:"+pminute+"  messageContent:"+messageContent);
			...
			if (!TextUtils.isEmpty(munite) && !TextUtils.isEmpty(cycle)) {
				...
				SLog.logI("定時推送广播开始分鐘 --> " + m);
				SLog.logI("定時推送广播周期 --> " + c);
				AlarmPushHelper.setAlarmPush(context, m, c);
				SLog.logI("AlarmPushReceiver had run when system start!");
			}
		}
