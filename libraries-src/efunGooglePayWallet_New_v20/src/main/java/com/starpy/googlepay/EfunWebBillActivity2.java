...
public abstract class EfunWebBillActivity extends Activity {
	...
	*
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (EfunStringUtil.isNotEmpty(efunWebPayParams)) {
			//官网储值
			if (EfunStringUtil.isNotEmpty(payPreferredUrl)) {
				efunWebPayReferredUrl = payPreferredUrl + EfunDomainSite.EFUN_MORE_GW_PAY + efunWebPayParams;
			}
			if (EfunStringUtil.isNotEmpty(paySpareUrl)) {
	...
	* <p>Description: 開始加載網頁</p>
	...
	public void loadWebViewContent() {
				...
				if (EfunStringUtil.isEmpty(efunWebPayReferredUrl) && EfunStringUtil.isEmpty(efunWebPaySpareUrl)) {
...
//				settings.setUseWideViewPort(true);
				...
				_efunWebView.setWebViewClient(new WebViewClient() {
					...
					public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
						...
						//如果首选域名访问超时，则转向备用域名
						if ((WebViewClient.ERROR_TIMEOUT == errorCode || WebViewClient.ERROR_CONNECT == errorCode) && failingUrl.contains(EfunWebBillActivity.this.payPreferredUrl) && EfunStringUtil.isNotEmpty(EfunWebBillActivity.this.efunWebPaySpareUrl)) {
