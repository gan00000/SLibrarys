...
import com.efun.util.IabHelper.OnIabPurchaseFinishedListener;
...
public class EfunPayRequest {
	...
	*/
	public static void requestSendStone2222222(final Context context,final Purchase purchase,final OnIabPurchaseFinishedListener mPurchaseListener){
		...
		if (null != walletBean) {
			...
			if (EfunStringUtil.isNotEmpty(purchaseData)) {
				...
				try {
					...
					if (EfunStringUtil.isNotEmpty(efunDeveloperPayload)) {
	...
	*/
	 ...
	 * @date 2016年1月14日
	 */
	private static void serverResponseProcess(final Context context, final OnIabPurchaseFinishedListener mPurchaseListener, final Purchase purchase,
