...
import com.efun.googlepay.efuntask.EfunPayRequest;
...
public class IabHelper {
	...
	String mDebugTag = "efun_IabHelper";
	...
	/**
	 * @return the mAsyncInProgress
	 */
	public boolean isAsyncInProgress() {
		return mAsyncInProgress;
	}
	public void setAsyncInProgress(boolean inProgress) {
		this.mAsyncInProgress = inProgress;
	}
	...
	//EfunVerifyTask verifyTask = new EfunVerifyTask();
	...
	private static final String TAG = "efun_IabHelper";
	...
	public void startSetup(final OnIabSetupFinishedListener listener) {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					...
					if (response != BILLING_RESPONSE_RESULT_OK) {
						...
						Log.d(BasePayActivity.TAG, "Error checking for billing v3 support.");
		...
		if (resolveInfos == null) {
			Log.d(BasePayActivity.TAG, "resolveInfos为null,找不到google play 远程服务");
		}
	...
	public void launchPurchaseFlow(BasePayActivity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener) {
		launchPurchaseFlow(act, sku, "",requestCode, listener, "");
	}

	public synchronized void launchPurchaseFlow(BasePayActivity act, String sku, String efunOrderId, int requestCode, OnIabPurchaseFinishedListener listener, String extraData) {
		launchPurchaseFlow(act, sku, efunOrderId, ITEM_TYPE_INAPP, requestCode, listener, extraData);
	}

	public void launchSubscriptionPurchaseFlow(BasePayActivity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener) {
		launchSubscriptionPurchaseFlow(act, sku, requestCode, listener, "");
	}

	public void launchSubscriptionPurchaseFlow(BasePayActivity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener, String extraData) {
	 ...
	 */
	public void launchPurchaseFlow(BasePayActivity act, String sku, String efunOrderId, String itemType, int requestCode, OnIabPurchaseFinishedListener listener,
			String extraData) {
		// checkNotDisposed();
	...
	/*	if (EfunGooglePayService.getPayActivity() != null && EfunGooglePayService.getPayActivity().getPrompt().isCancel()) {
		...
		try {
			...
			if (act == null || act.isPurchaseCancel()) {//判断购买的时候，是否已经取消了
				Log.d(mDebugTag, "purchase is cancel");
				return ;
			}
		...
		/*	if (EfunGooglePayService.getPayActivity() != null && EfunGooglePayService.getPayActivity().getPrompt().isCancel()) {
			...
			if (!TextUtils.isEmpty(previousOrderId) && previousOrderId.equals(efunOrderId)) {
				Log.d(mDebugTag, "efun orderId 重复");
				act.getPayPrompt().complain("error,please try again");
				return;
			}
			if (!TextUtils.isEmpty(efunOrderId)) {
				EfunPayHelper.saveCurrentOrderId(act, efunOrderId);
			}
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		...
		// end of async purchase operation
		flagEndAsync();
		// EfunVerifyTask mVerifyTask = null;
		...
		if (resultCode == Activity.RESULT_OK && responseCode == BILLING_RESPONSE_RESULT_OK) {
			if (EfunGooglePayService.getPayActivity() != null) {
				EfunGooglePayService.getPayActivity().setGooglePayOk(true);
			}
			showProgressDialog("Do not cancel...");
		...
		//	verifyTask.verifyOrder(context, mPurchaseListener, purchaseData, dataSignature, mPurchasingItemType);
	         ...
	         /*   if (purchase != null) {
					EfunPayRequest.requestSendStone(context, purchase, mPurchaseListener);
				}*/
	            if (mPurchaseListener != null) {
	...
	public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus, final QueryInventoryFinishedListener listener) {
		...
		if (mAsyncInProgress) {
			...
			Log.d(BasePayActivity.TAG, "mAsyncInProgress is true");
			return;
		}
		Log.d(BasePayActivity.TAG, "query start...");
		...
		(new Thread(new Runnable() {
			public void run() {
				...
				flagEndAsync();
				Log.d(BasePayActivity.TAG, "query end...");
	...
	void checkSetupDone(String operation) {
		if (!mSetupDone) {
			// basePayActivity.getPrompt().complainCloseAct("Google儲值未初始化完成，請重新進入");
			if (EfunGooglePayService.getPayActivity() != null) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					Log.d(BasePayActivity.TAG, "is success:" + result);
				} catch (RemoteException e) {
	...
	/*
	 * public void getPuchase(){ new Thread(new Runnable() {
	 * 
	 * @Override public void run() { try { Bundle bundle =
	 * mService.getPurchases(3, context.getPackageName(), "inapp",
	 * "oaggembndnldceifohhcdbjm.AO-J1OyFnG8YfBE9zKwSWd6QWgK-kcNEdeR78c_DShcv3Ehbbrn-hc1ukKkb1K73uUu327rdsj9RMmE5JqQ1fLIZsIVdvriVpGtfxmx_tN-y2L9e1XYRnLA"
	 * ); Log.d(TAG, "bundle:" + bundle.toString()); Log.d(TAG, "bundle:" +
	 * bundle.getString("RESPONSE_CODE", "")); Log.d(TAG, "bundle:" +
	 * bundle.toString());
	 * 
	...
	private void dismissProgressDialog() {
		if (EfunGooglePayService.getPayActivity() != null && EfunGooglePayService.getPayActivity().getPayPrompt() != null) {
			EfunGooglePayService.getPayActivity().getPayPrompt().dismissProgressDialog();
		}
	}

	private void showProgressDialog(String string) {
		if (EfunGooglePayService.getPayActivity() != null && EfunGooglePayService.getPayActivity().getPayPrompt() != null) {
			EfunGooglePayService.getPayActivity().getPayPrompt().showProgressDialog(string);
		}
	}
