		...
		if (adsHttpParams == null) {
		 ...
		 if (s2sListener == null) {
			...
			try {
				...
				if (EfunStringUtil.isNotEmpty(s2slistener) && s2slistener.startsWith("com.")) {
	
					@SuppressWarnings("unchecked")
					Class<S2SListener> clazz = (Class<com.efun.ads.callback.S2SListener>) Class.forName(s2slistener);
					if (clazz != null) {
...
//				AdsHelper.initParams(this, adsHttpParams);
	...
	private boolean exitsLocalAdsCode() {
		...
		if (EfunStringUtil.isNotEmpty(advertisersResult_new) && advertisersResult_new.equals(SPUtil.ADS_ONLYONCE_CODE)) {
