...
import com.starpy.base.beans.DynamicVersionBean;
import com.starpy.base.beans.UrlBean;
import com.starpy.base.beans.UrlFileContent;
...
//		String localContentFileDir = mContext.getFilesDir().getAbsolutePath() + File.separator + "efun" + File.separator;
		...
		String contentFileName = EfunFileUtil.getFileName(contentFileUrl);
...
//		String versionContent = requestVersionContent(mVersionCodeFileUrl);//读取远程服务器版本号文件内容
		...
		EfunLogUtil.logD("versionContent:" + versionContent);
		if (!TextUtils.isEmpty(versionContent)) {//保存一下
			if (isPlatform) {
				SPUtil.saveSimpleInfo(mContext, SPUtil.EFUN_FILE, SPUtil.EFUN_APP_PLATFORM_DYNAMIC_VERSION_CONTENT, versionContent);
			} else {
				SPUtil.saveSimpleInfo(mContext, SPUtil.EFUN_FILE, SPUtil.EFUN_GAME_DYNAMIC_VERSION_CONTENT, versionContent);
			}
		}
...
//				boolean isFinish = downLoadUrlFile(mUrlFileUrl, localContentFilePath);// 下载域名内容文件
	...
	*/
	private UrlFileContent readUrlFileContent(Context context,String filePath){
		...
		try {
			localContent = EfunFileUtil.readFileByByte(context, filePath);
			EfunLogUtil.logD("密文:" + localContent);
			if (!TextUtils.isEmpty(localContent)) {
				localVersionContentMd5 = SStringUtil.toMd5(localContent, false);
				EfunLogUtil.logD("密文MD5:" + localVersionContentMd5);
				...
				if (!TextUtils.isEmpty(localPlaintext)) {
					...
					EfunLogUtil.logD("域名文件 VersionCode:" + localVersionCode);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
