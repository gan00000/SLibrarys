...
//		String versionContent = requestVersionContent(mVersionCodeFileUrl);//读取远程服务器版本号文件内容
		...
		if (!TextUtils.isEmpty(versionContent)) {//保存一下
			if (isPlatform) {
				EfunDatabase.saveSimpleInfo(mContext, EfunDatabase.EFUN_FILE, EfunDatabase.EFUN_APP_PLATFORM_DYNAMIC_VERSION_CONTENT, versionContent);
			} else {
				EfunDatabase.saveSimpleInfo(mContext, EfunDatabase.EFUN_FILE, EfunDatabase.EFUN_GAME_DYNAMIC_VERSION_CONTENT, versionContent);
			}
		}
	...
	private UrlFileContent readUrlFileContent(Context context,String filePath){
		...
		try {
			...
			if (!TextUtils.isEmpty(localContent)) {
				localVersionContentMd5 = EfunStringUtil.toMd5(localContent, false);
