...
public class EfunLogFileUtil {
    ...
    protected static void uploadLog(final Context cxt){
        //没有网络时候就退出
        if(!EfunLocalUtil.isNetworkAvaiable(cxt)){
            return;
        }
        ...
        if(!TextUtils.isEmpty(strLogFilePath)) {
            ...
            if(logFile.exists() && EfunUploadLogAsync.checkThreadInstance()){
                if(logFile.length() > MIN_UPLOAD_SIZE && logFile.length() < MAX_MOBILE_UPLOAD_SIZE){
                    ...
                    if(EfunLocalUtil.isWifiAvailable(cxt)){
                        new EfunUploadLogAsync(cxt, logFile).start();
    ...
    private static class EfunUploadLogAsync extends Thread {
        ...
        public void run() {
            ...
            String preferredUploadUrl = EfunResourceUtil.findStringByName(mContext.get(), KEY_LOG_UPLOAD_PREFERRED_URL);
            String spareUploadUrl = EfunResourceUtil.findStringByName(mContext.get(), KEY_LOG_UPLOAD_SPARE_URL);
            if(EfunStringUtil.isEmpty(preferredUploadUrl) && EfunStringUtil.isEmpty(preferredUploadUrl)){
        ...
        String doRequest(String preferredUrl, String sparedUrl) {
            ...
            if(EfunStringUtil.isEmpty(response) && EfunStringUtil.isNotEmpty(sparedUrl)) {
