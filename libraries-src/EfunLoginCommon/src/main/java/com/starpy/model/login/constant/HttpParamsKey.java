/**
 * 
 */
package com.starpy.model.login.constant;


/**
* <p>Title: HttpParamsKey</p>
* <p>Description: 接口请求参数key字符串常量</p>
* <p>Company: EFun</p> 
* @author GanYuanrong
* @date 2014年8月22日
*/
public class HttpParamsKey {

	public static final String loginName = "name";
	public static final String loginPwd = "pwd";
	public static final String timestamp = "timestamp";
	public static final String gameCode = "gameCode";
	public static final String systemVersion = "systemVersion";
	public static final String deviceType = "deviceType";
	public static final String signature = "signature";
	public static final String email = "email";
	public static final String mac = "mac";
	public static final String appKey = "appKey";
	public static final String ip = "ip";
	public static final String registPlatform = "registPlatform";
	public static final String operatingSystem = "operatingSystem";
	public static final String advertiser = "advertiser";
	public static final String androidid = "androidid";
	public static final String imei = "imei";
	public static final String newPwd = "newPwd";
	public static final String loginid = "loginid";
	public static final String partner = "partner";
	public static final String referrer = "referrer";
	public static final String params = "params";
	public static final String appPlatform = "appPlatform";
	public static final String language = "gameLanguage";
	public static final String phone = "phone";
	public static final String code = "code";
	public static final String typeNames = "typeNames";
	public static final String userId = "userId";
	public static final String auth="auth";
}
