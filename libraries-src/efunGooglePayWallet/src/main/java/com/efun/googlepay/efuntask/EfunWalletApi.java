...
public class EfunWalletApi {
	...
	*/
	...
	//	List<NameValuePair> params = new ArrayList<NameValuePair>();
		...
		if (EfunStringUtil.isNotEmpty(userId)&& EfunStringUtil.isNotEmpty(creditId) && EfunStringUtil.isNotEmpty(goodsId) 
				&& EfunStringUtil.isNotEmpty(moneyType) && EfunStringUtil.isNotEmpty(serverCode) && EfunStringUtil.isNotEmpty(gameCode) 
				&& EfunStringUtil.isNotEmpty(payFrom)&& EfunStringUtil.isNotEmpty(payType)) {
			
			String localMacAddress = (null == EfunLocalUtil.getLocalMacAddress(payActivity)?"":EfunLocalUtil.getLocalMacAddress(payActivity));
			String localImeiAddress = (null == EfunLocalUtil.getLocalImeiAddress(payActivity)?"":EfunLocalUtil.getLocalImeiAddress(payActivity));
			String localIpAddress = (null == EfunLocalUtil.getLocalIpAddress(payActivity)?"":EfunLocalUtil.getLocalIpAddress(payActivity));
			String localAndroidId = (null == EfunLocalUtil.getLocalAndroidId(payActivity)?"":EfunLocalUtil.getLocalAndroidId(payActivity));
			...
			params.put("gameVersion", EfunPayHelper.getVersionName(payActivity));//添加包版本名称参数
			
			params.put("roleId", orderBean.getRoleId());
			params.put("googlepayversion", BasePayActivity.GOOGLE_PAY_VERSION);
			params.put("sign", EfunPayHelper.getLoginSign(payActivity));
...
//		List<NameValuePair> verifyParams = new ArrayList<NameValuePair>();
		...
		verifyParams.put("gameVersion", EfunPayHelper.getVersionName(payActivity));
		...
		/**文件记录丢单情况的server跟踪标识**/
		if("isSharedPreferences".equals(isSharedPreferencesMark)){
			verifyParams.put("missBill", "201312161625");
		}
