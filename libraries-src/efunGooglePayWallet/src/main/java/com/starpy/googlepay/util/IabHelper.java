...
import com.starpy.googlepay.task.EfunVerifyTask;
...
public class IabHelper {
	...
	public static final String mDebugTag = "efun_IabHelper";
	 ...
	 */
	public IabHelper(BasePayActivity ctx) {
		// basePayActivity = ctx.getApplicationContext();
		basePayActivity = ctx;
...
/*	public void enableDebugLogging(boolean enable, String tag) {
	...
	public void startSetup(final OnIabSetupFinishedListener listener) {
		...
		mServiceConn = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				String packageName = basePayActivity.getPackageName();
				try {
		...
		List<ResolveInfo> resolveInfos = basePayActivity.getPackageManager().queryIntentServices(serviceIntent, 0);
		if (resolveInfos != null && !resolveInfos.isEmpty()) {
			// service available to handle that Intent
			basePayActivity.bindService(serviceIntent, mServiceConn, Context.BIND_AUTO_CREATE);
		} else {
	...
	public void dispose() {
		...
		if (mServiceConn != null) {
			...
			if (basePayActivity != null) {
				try {
					basePayActivity.unbindService(mServiceConn);
				} catch (Exception e) {
					e.printStackTrace();
				} finally {
					mServiceConn = null;
	...
	public void launchPurchaseFlow(Activity act, String sku, String itemType, int requestCode, OnIabPurchaseFinishedListener listener,
		...
		try {
			...
			Bundle buyIntentBundle = mService.getBuyIntent(3, basePayActivity.getPackageName(), sku, itemType, extraData);
			...
			if (!TextUtils.isEmpty(previousOrderId)) {
				if (previousOrderId.equals(basePayActivity.getGoogleOrderBean().getOrderId())) {
					Log.d(mDebugTag, "efun orderId 重复");
					basePayActivity.getPrompt().complain("error,please try again");
					return;
				}
			}
			PayHelper.saveCurrentOrderId(act, basePayActivity.getGoogleOrderBean().getOrderId());
	...
	public boolean handleActivityResult(int requestCode, int resultCode, Intent data) {
		basePayActivity.getPrompt().showProgressDialog("Do not cancel...");
		IabResult result;
	...
	void consume(Purchase itemInfo) throws IabException {
		...
		try {
			...
			int response = mService.consumePurchase(3, basePayActivity.getPackageName(), token);
			if (response == BILLING_RESPONSE_RESULT_OK) {
	...
	void checkSetupDone(String operation) {
		if (!mSetupDone) {
			...
			basePayActivity.getPrompt().complainCloseAct("Google IAB set up error，please try again");
	...
	int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException {
		...
		logDebug("Package name: " + basePayActivity.getPackageName());
		boolean verificationFailed = false;
		String continueToken = null;
		// EfunVerifyTask verifyTask = new EfunVerifyTask();
		do {
			logDebug("Calling getPurchases with continuation token: " + continueToken);
			Bundle ownedItems = mService.getPurchases(3, basePayActivity.getPackageName(), itemType, continueToken);
	...
	int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws RemoteException, JSONException {
		...
		Bundle skuDetails = mService.getSkuDetails(3, basePayActivity.getPackageName(), itemType, querySkus);
