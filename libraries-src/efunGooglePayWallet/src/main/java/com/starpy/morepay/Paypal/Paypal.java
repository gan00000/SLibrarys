...
public class Paypal extends TaskRequest {
	...
	*//*
	public void paypalCheck(String userid, String gameCode){
		...
		final String efunLocalOrderId = SPUtil.getSimpleString(activity, GooglePayContant.EFUNFILENAME, PAYPAL_ORDER_ID_KEY);
		map.put("efunOrderId", efunLocalOrderId);
		this.setMap(map);
		if (SStringUtil.isEmpty(this.getPreferredUrl())) {
		...
		this.setRequestCallBack(new RequestCallBack() {
			...
			public void onRequestFinish(String result) {
				...
				try {
					...
					if ("0000".equals(code)) {
//						if (SStringUtil.isNotEmpty(efunOrderId)) {
//							SPUtil.saveSimpleInfo(activity, GooglePayContant.EFUNFILENAME, PAYPAL_ORDER_ID_KEY, efunOrderId);
//						}
