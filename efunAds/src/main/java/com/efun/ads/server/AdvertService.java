...
public class AdvertService {
	...
	public  void s2sWaitForGAAndSendPost(final S2SListener mS2sListener,Handler handler, Map<String, String> params){
		...
		if (EfunStringUtil.isNotEmpty(localCode)) {
		...
		if(EfunStringUtil.isEmpty(advertiser) && EfunStringUtil.isEmpty(partner) && EfunStringUtil.isEmpty(referrer)){
...
//		params.put("efunthirdplat", this.efunThirdPlat);
		...
		if (EfunStringUtil.isEmpty(this.efunThirdPlat)) {
			...
			if (EfunStringUtil.isAllEmpty(efunThirdPlatPreferredUrl,efunThirdPlatSpareUrl)) {
				Log.e("efunLog","广告url设置有误");
				return;
			}
			if (EfunStringUtil.isNotEmpty(efunThirdPlatPreferredUrl)) {
				efunThirdPlatPreferredUrl = efunThirdPlatPreferredUrl + EfunDomainSite.EFUN_ADS_THIRDPLAT;
			}
			if (EfunStringUtil.isNotEmpty(efunThirdPlatSpareUrl)) {
		...
		*//**
		 ...
		 * 1006:该设备已经安装过该游戏
		 *//*
		if (null != this.context && EfunStringUtil.isNotEmpty(adsCode)) {
	...
	public  Map<String, String> initAdsPostParams(Context context) {
		...
		String signature  = EfunStringUtil.toMd5(appKey+ timeStamp + localMacAddress + gameCode + localImeiAddress + localIpAddress + localAndroidId, false);
		...
		params.put("osVersion", EfunLocalUtil.getOsVersion());
		params.put("phoneModel", EfunLocalUtil.getDeviceType());
		...
		Log.i("efunLog", "mac:" + localMacAddress +
				...
				",osVersion:" + EfunLocalUtil.getOsVersion() +
				",phoneModel:" + EfunLocalUtil.getDeviceType());
