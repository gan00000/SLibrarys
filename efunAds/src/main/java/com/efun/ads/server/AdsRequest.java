...
//		Log.d("efunLog", "ads params:" + adsHttpParams.toString());
	...
	private void checkResponeCodeAndSave(final S2SListener mS2sListener, Handler handler, final String adsCode) {
		...
		if (null != this.context && EfunStringUtil.isNotEmpty(adsCode) && EfunStringUtil.isNotEmpty(adsCode.trim()) && !"null".equals(adsCode)) {
...
//			}
	...
	private boolean isNeedToWaitGA() {
		...
		if (TextUtils.isEmpty(SPUtil.takeAdvertisersName(context, ""))
				...
				&&EfunStringUtil.isEmpty(adsHttpParams.getAdvertiser()) 
				//&& EfunStringUtil.isEmpty(adsHttpParams.getPartner()) 
				&& EfunStringUtil.isEmpty(adsHttpParams.getReferrer())
				) {
			return true;
		}
		return false;
