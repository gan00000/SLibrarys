...
public class EfunPushReceiver extends BroadcastReceiver {
	...
	private void processClickNotificationIntent(Context context) {
		...
		boolean isEmpty = EfunFirebaseMessagingService.getNotificationMessages().isEmpty();
		if (!isEmpty) {
			int size = EfunFirebaseMessagingService.getNotificationMessages().size();
			NotificationMessage lastN = EfunFirebaseMessagingService.getNotificationMessages().get(size - 1);

			//处理点击后的事件
			if (EfunFirebaseMessagingService.instanceMessageDispatcher(context) != null && lastN != null) {//额外处理
					//返回true表示不再传递处理，返回false表示继续传递事件给默认点击处理器处理
				if (EfunFirebaseMessagingService.instanceMessageDispatcher(context).onClickNotification(context, lastN.getContent(), lastN.getData())) {
					EfunFirebaseMessagingService.getNotificationMessages().clear();
					return;
				}
			}
