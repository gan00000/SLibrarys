...
import com.starpy.google.bean.SFirebaseKey;
...
public class SFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        MessageDispatcher md = SFirebaseMessagingService.instanceMessageDispatcher(getApplicationContext());
        if (md != null){
            if (md.onMessage(getApplicationContext(),body,data)) {
                return;//是否消费掉该消息，若消费掉，则返回true,改消息不再传递给默认处理器
            }
        }
        if (remoteMessage.getData() != null && !TextUtils.isEmpty(remoteMessage.getData().get(SFirebaseKey.efun_not_show_message))){
            if (md != null) {
...
//            Bdown.excuteSpecialThing(getApplicationContext(),data);
            ...
            n.setContent(body);
            if (data != null){
                n.setClickOpenUrl(data.get(SFirebaseKey.efun_click_open_url));
            }
            n.setData(data);
